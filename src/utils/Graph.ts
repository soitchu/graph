import { GraphExtension } from "./GraphExtension.js";
...
export class Graph {
    ...
    bifurHelper: OffscreenCanvas;
    bifurHelperCtx: OffscreenCanvasRenderingContext2D;
    backBufferCanvas: OffscreenCanvas;
    backBufferCtx: OffscreenCanvasRenderingContext2D;
    ...
    extensions: Array<GraphExtension> = [];
    lyapHelper: OffscreenCanvas;
    lyapHelperCtx: OffscreenCanvasRenderingContext2D;
    scale: number;
    elementary = 100;
    coordsSize = 0;
    scaleAnchor = {
    ...
    toDrawX: Array<GraphXY> = [];
    toDrawY: Array<GraphXY> = [];
    toDrawLines: Array<GraphLine> = [];
    toDrawLyapLines: Array<GraphLine> = [];
    toDrawCirles: Array<GraphPoint> = [];
    toDrawIneq: Array<GraphIneq> = [];
    toDrawLyapunov: Array<GraphLyapunov> = [];
    ...
    workersLyapunov: Array<Worker> = Array(navigator.hardwareConcurrency * 2);
    drawY = false;
    coordinatesScale = {
        x: 1,
        y: 1
    ...
    constructor(canvas: HTMLCanvasElement, height: number, width: number, drawY = false, coordinatesScale?: { x: number, y: number }) {
        if (coordinatesScale) {
    ...
    addGraph(config: GraphModes) {
        switch (config.type) {
            ...
            case "drawLines":
                ...
                this.toDrawLines.push(config as GraphLine);
                break;
            case "drawCircle":
    ...
    canvasToGraphCoords(x: number, y: number) {
        return [
    ...
    graphToCanvasCoords(x: number, y: number) {
        return [
    ...
    scaleUp(inc: number, self: Graph, x = 0, y = 0) {
        ...
        self.ctx.drawImage(
            ...
            startCoords[0],
            startCoords[1],
            Math.abs(endCoords[0] - startCoords[0]),
            Math.abs(endCoords[1] - startCoords[1]),
        );
    ...
    drawLyapunovLinear() {
        ...
        ctx.strokeStyle = window["strokeStyle"] ?? "black";
    ...
    drawGraphX() {
        ...
        let ctx = this.ctx;
        ctx.strokeStyle = window["strokeStyle"] ?? "white";
    ...
    addExtension(ext: GraphExtension) {
    ...
    drawNums() {
        ...
        for (var i = 0; i < xAxisNums.length; i++) {
            let val: number | string = xAxisNums[i][0] / this.coordinatesScale.x;
        ...
        for (var i = 0; i < yAxisNums.length; i++) {
            let val: number | string = yAxisNums[i][0] / this.coordinatesScale.y;
            ...
            let textWidth = ctx.measureText(val as string).width;
    ...
    mouseWheelEvent(event, self: Graph) {
